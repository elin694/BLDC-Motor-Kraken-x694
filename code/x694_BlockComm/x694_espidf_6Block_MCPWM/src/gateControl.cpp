...
#include "GC.h"
#include <cinttypes> // Required for PRIu32
...
void mcpwmSetup(int startingTargetSector, volatile uint32_t * bPeriod_pass_by_function){
    ...
    BTimerOnSync.count_value = static_cast<uint32_t>(global.blockPeriod-(estimatedI2CReadTimeInMicros*µsToTicks)); //is the starting phaseOffset
    LTimerOnSync.count_value = static_cast<uint32_t>(lowGateLevelCycle[startingTargetSector] *(*bPeriod_pass_by_function))+2; 
    ...
    ESP_ERROR_CHECK(esp_intr_enable(sixBlockISR));
   for (int i= 0; i<3; i++){ 
...
void initializeISRsAndSyncs(){
    ...
    ESP_ERROR_CHECK(mcpwm_new_soft_sync_src(&tripleHighTriggerSetup, &tripleHighTrigger));
    ...
    tripleHighOnSync.sync_src = tripleHighTrigger; 
    BTimerOnSync.sync_src = BTimerTrigger; 
    LTimerOnSync.sync_src = LTimerTrigger;

    ESP_ERROR_CHECK(mcpwm_timer_set_phase_on_sync(motorH[0].timer, &tripleHighOnSync));
    ESP_ERROR_CHECK(mcpwm_timer_set_phase_on_sync(motorH[1].timer, &tripleHighOnSync));
    ESP_ERROR_CHECK(mcpwm_timer_set_phase_on_sync(motorH[2].timer, &tripleHighOnSync));
...
void firstPreload(phaseMcpwm * motorHigh, phaseMcpwm * motorLow, int startingTargetSector, uint32_t bPeriod_pass_by_function){
    ...
    ESP_ERROR_CHECK(mcpwm_soft_sync_activate(tripleHighTrigger));
}
...
void IRAM_ATTR phaseSwitching(void * returnValue){
   #if (lowSideGroup == 1)
   ...
   //checks to see if any of ther flags are triggered
    ...
    #endif 
        executeGates(&tripleHighTrigger);
...
void IRAM_ATTR executeGates(mcpwm_sync_handle_t * trigger){
    ESP_ERROR_CHECK(mcpwm_soft_sync_activate(*trigger));
    // for(int i =0; i <3; i++){
    //     switch (state) {
    //         case -1: //sink current
    //             if ( -1 != previousState){
